...
function combineVersion(major, minor, patch, preRelease2) {
	...
	if (preRelease2) return `${mainVersion2}-${preRelease2}`;
	return mainVersion2;
}
...
function parseTildes(range) {
	return range.trim().split(/\s+/).map((rangeVersion) => {
		return rangeVersion.replace(parseRegex(tilde), (_, major, minor, patch, preRelease2) => {
			...
			else if (preRelease2) return `>=${major}.${minor}.${patch}-${preRelease2} <${major}.${+minor + 1}.0-0`;
...
function parseXRanges(range) {
	return range.split(/\s+/).map((rangeVersion) => {
		return rangeVersion.trim().replace(parseRegex(xRange), (ret, gtlt2, major, minor, patch, preRelease2) => {
			...
			} else if (isXMinor) return `>=${major}.0.0${preRelease2} <${+major + 1}.0.0-0`;
			else if (isXPatch) return `>=${major}.${minor}.0${preRelease2} <${major}.${+minor + 1}.0-0`;
			return ret;
		});
	}).join(" ");
}
...
function satisfy(version, range) {
	...
	const comparators = parseRange(range).split(" ").map((rangeVersion) => parseComparatorString(rangeVersion)).join(" ").split(/\s+/).map((comparator2) => parseGTE0(comparator2));
...
//#endregion
//#region \0virtual:__federation_fn_import
var moduleMap = {};
...
async function getSharedFromRuntime(name, shareScope) {
	...
	if (globalThis?.__federation_shared__?.[shareScope]?.[name]) {
		...
		if (!!requiredVersion) {
			...
			if (versionKey) module = await (await versionObj[versionKey].get())();
			else console.log(`provider support ${name}(${versionKey}) is not satisfied requiredVersion(\${moduleMap[name].requiredVersion})`);
		} else module = await (await versionObj[Object.keys(versionObj)[0]].get())();
	}
	if (module) return flattenModule(module, name);
}
async function getSharedFromLocal(name) {
	if (moduleMap[name]?.import) return flattenModule(await (await moduleMap[name].get())(), name);
